...
def update_table(request):
    if request.method == 'POST':
        try:
            ...
            # Inicializando a planilha Excel
            wb = Workbook()
            ws = wb.active
            ws.title = "Ações"
            ...
            # Adiciona cabeçalhos
            headers = [
                'CIS Control', 'CIS Sub-Control', 'Tipo de Ativo', 'Função de Segurança',
                'Título', 'Descrição', 'NIST CSF', 'Nome da Subcategoria', 'Ação'
            ]
            ws.append(headers)
            ...
            if request.POST.get('save') == 'temporary':
                ...
                for item in framework.objects.all():
                    ...
                    if action:
                        ...
                        ws.append([
                            item.cis_control,
                            item.cis_sub_control,
                            item.tipo_de_ativo,
                            item.funcao_de_seguranca,
                            item.titulo,
                            item.descricao,
                            item.nist_csf,
                ...
                # Após salvar permanentemente, apaga a tabela temporária
                TemporaryActionModel.objects.filter(nome=nome_cliente).delete()
                
                messages.success(request, 'Tabela atualizada e salva com sucesso!')

            # Salva o arquivo Excel no diretório `media`
            file_name = f"{nome_cliente}_{today}.xlsx"
...
@csrf_exempt
def update_action(request, item_id):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            action = data.get('action')
            item = ActionModel.objects.get(id=item_id)
            item.acao = action
            item.save()
